...
server/          Backend Express (JS, CommonJS)
  ...
  career.js         Règles pures du Mode Carrière (stats par paliers de 100, énergie, 10 tours, rang de sortie)
  careerRounds.js   Lien session solo ↔ gameState ↔ career (démarrage/règlement des rounds d'étude et de sortie)
...
- **Mode Carrière** (spécification : `us/carriere-v1.md`) : `POST /api/career` (nouvelle carrière), `GET /api/career`, `POST /api/career/rest`, `POST /api/career/study { stat }`, `POST /api/career/release` (démarre le titre suivant de l'album de 6 : `career.album { done, total }` ; `career.release { score, maxScore, grade, tracks }` n'apparaît qu'après le 6e titre), toutes sous `X-Solo-Session` et renvoyant `{ career, round }` (`round` : `{ kind, stat, state }` ou `null`). Un round de carrière se joue via `/api/guess`, `/api/skip` et `/audio/track` : la dernière réponse d'un round ajoute `career` (mis à jour). Les paliers d'un round viennent de `gameState.resetState(key, tiersSeconds)` (défaut inchangé), `career.suggestionCount` fixe le nombre de suggestions de recherche que le client applique. Erreurs stables : `NO_CAREER` (404), `INVALID_STAT` (400), `ROUND_IN_PROGRESS`, `NO_ENERGY`, `RELEASE_DUE`, `RELEASE_NOT_DUE`, `CAREER_FINISHED` (409). Le titre du round n'est renvoyé qu'une fois le round fini ; clé de round `<sessionId>:career:<songId>`.

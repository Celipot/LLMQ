...
## Pool de titres
Les études et la sortie tirent dans la **discographie du personnage** (A・ZU・NA, 82 titres : solos d'Ayumu, de Shizuku et de Setsuna, unité A・ZU・NA, groupe Nijigasaki, cf. `solo.md` §1). Il n'y a pas de champ « personnage » dans `songs.json` : la discographie se dérive du champ `artist` (solos : `Ayumu Uehara (CV: Aguri Onishi)`, `Shizuku Osaka (CV: Kaori Maeda)`, `Setsuna Yuki (CV: Tomori Kusunoki)` et `Setsuna Yuki (CV: Coco Hayashi)` ; unité : `A・ZU・NA` ; groupe : la chaîne exacte du groupe Nijigasaki, à relever dans les données). Un filtre dans `career.js` suffit pour la v1. Le tirage des études évite les titres déjà trouvés dans la carrière. L'album, lui, est tiré des **titres trouvés en étude** (le carnet) : étudier prépare la sortie. S'il y en a moins de 6, il est complété par des titres tirés au hasard dans la discographie ; un titre n'apparaît jamais deux fois sur le même album.
